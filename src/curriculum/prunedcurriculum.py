...
class PrunedCurriculum(ObjectOrientedCurriculum):

    repetitions = None #Number of walks
    randomSteps = None #Number of random steps

    def __init__(self, seed=12345, agent=None):
        super(PrunedCurriculum, self).__init__(seed=seed, agent=agent)
    ...
    def generate_curriculum(self, target_task, sourceFolder, workFolder, thresholdTask=None):
        ...
        if thresholdTask is None:
            if target_task.get_domain_task() == 'HFOTask':
                ...
                thresholdTask = 1  # 40
                self.agent.alpha = 0.9  # Grid
                self.repetitions = 3
                self.randomSteps = 2

        super(PrunedCurriculum, self).generate_curriculum(target_task=target_task, sourceFolder=sourceFolder,
    ...
    def generate_curriculum_from_tasks(self, target_task, taskList, thresholdTask=None):
        ...
        if thresholdTask is None:
            if target_task.get_domain_task() == 'HFOTask':
                ...
                thresholdTask = 1
                self.agent.alpha = 0.9  # Grid
        super(PrunedCurriculum, self).generate_curriculum_from_tasks(target_task=target_task, taskList=taskList,
